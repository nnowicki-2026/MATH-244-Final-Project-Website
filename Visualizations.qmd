...
critic_review <- pivot_longer(movies, 
                        ...
                        names_to = "group",
                        values_to = "value") #grouping the rating columns to make the bax plot
#critic review side-by-side box plot
...
#| message: false
#| warning: false
#scatterplot for number of theaters showing the movie and opening weekend profit
ggplot(data = movies,
       mapping = aes(x = TheatersOpenWeek, y = OpeningWeekend)) +
  geom_point() +
  geom_smooth(method = "glm", #exponential fit
              formula = y ~ x,
...
set.seed(123) #taking a random sample for the k0means clusterin
kmeans_result <- kmeans(budget_data, centers = 5) #state the number of clusters
kmeans_result$centers

movies$Cluster <- as.factor(kmeans_result$cluster)

budget_ranges <- movies %>% #group the cluster groups
  group_by(Cluster) %>%
  summarise(
    Count = n(),
...
print(budget_ranges) #show the ranges of the budgets in each cluster
print(kmeans_result$centers)

cluster_means <- movies %>%
  group_by(Cluster) %>%
  summarise(mean_opening = mean(OpeningWeekend, na.rm = TRUE)) %>%
  arrange(mean_opening) #order the clusters in increasing budgets

#clustering side-by-side box plots, visualization
...
#| message: false
#| warning: false
#multilinear regression with profitability as outcome
...
#| message: false
#| warning: false
#| #multilinear regression with world gross as outcome
